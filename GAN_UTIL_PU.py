# -*- coding: utf-8 -*-
"""
Created on Tue May 14 16:25:37 2019
...
from PU_DATA import PU_DS
'''
Creates directory for test. Returns the dir path to GAN RUN. Calls README_MAKE
and writer_make 
'''
...
'''
Create CSV files with headers to log loss and accuracy values in main GAN RUN
'''        
def writer_make(path,date):
    ...
    with open(log_path,'a') as log, open(acc_path,'a') as acc, open(class_path,'a') as cla:
        Fieldnames1 = ['Epoch','Global_Step','Disc_Loss','Gen_Loss','Real_Loss','Fake_Loss']
        Fieldnames2 = ['Epoch','Global_Step','D_true_acc','G_Precision',
                       'D_true_syn','D_false_syn','D_false_real','G_confusion']
        Fieldnames3 = ['Epoch','Global_Step','Class_Loss','Class_Acc']
        writer1 = csv.DictWriter(log,Fieldnames1)
        writer1.writeheader()
        writer2 = csv.DictWriter(acc,Fieldnames2)
        writer2.writeheader()
        writer3 = csv.DictWriter(cla,Fieldnames3)
        writer3.writeheader()
...
def loss_plots(path):
    ...
    ax[1].plot(df['Global_Step'],df['Disc_Loss'])
    ax[1].set(title='D_Loss, Real and Synthetic Outputs',xlabel='Global Steps',ylabel='Loss')
...
def acc_plots(path):
    ...
    ax[0].plot(df['D_true_acc'])
    ax[0].plot(df['D_true_syn'])
    ax[0].set(title='Discriminator Performance True',xlabel='Global Steps',ylabel='Accuracy')
    ax[0].set_autoscaley_on(0)
    ax[0].legend()
    
    ax[1].plot(df['G_Precision'])
    ax[1].plot(df['G_confusion'])
    ax[1].set(title='Generator Performance',xlabel='Global Steps',ylabel='Accuracy')
    ax[1].set_autoscaley_on(0)
    ax[1].legend()
    
    ax[2].plot(df['D_false_real'])
    ax[2].plot(df['D_false_syn'])
    ax[2].set(title='False Indications',xlabel='Global Steps',ylabel='Percent')
    ax[2].set_autoscaley_on(0)
    ax[2].legend()
...
def make_dataset(data='',batch=100,random_state=42,split_size=0.3,
    ...
    X_train, X_test, y_train, y_test = train_test_split(Data, Lab,
    ...
    train_ds = tf.data.Dataset.zip((train_X,train_y)).shuffle(len(X_train)).batch(batch)
    ...
    test_ds = tf.data.Dataset.zip((test_X,test_y)).shuffle(len(X_test)).batch(batch)
