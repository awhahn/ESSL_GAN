# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 17:53:25 2019
...
class PU_DS(object):
    ...
    def __init__(self):
        ...
        data = sp.loadmat('PaviaU.mat')['paviaU'].astype('float32')
        self.data = np.reshape(data,[data.shape[0]*data.shape[1],data.shape[2]])
        
        lab =  sp.loadmat('PaviaU_gt.mat')['paviaU_gt']
        self.lab = np.reshape(lab,[lab.shape[0]*lab.shape[1],1]).astype('int64')
    ...
    def plot_data(self,dtype=None,n_components=None,size=20):
        ...
        for i,axes in enumerate(ax.flat):
            if i < len(Data):
                idx = np.random.randint(low=0,high=Data[i].shape[0],size=size)
                p = Data[i][idx,:]
    ...
    def get_weights(self,dtype='null'):
        ...
        mean = np.mean(ls)
        weights = [mean/ls[i] if mean/ls[i] > 1 else 1 for i in range(len(Data))]
        return weights
